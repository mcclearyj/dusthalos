...
randoms = glob.glob(os.path.join(
    catalog_dir, "sdss_bg_photoz2_randoms?.fits"
))
masked_randoms = glob.glob(os.path.join(
    catalog_dir, "prep_cat_sdss/rand_sdss_bg2_JOINED_catalog?.fits"
))
...
vstack(holder).write(
    os.path.join(catalog_dir, "sdss_bg_photoz2_randoms_stacked.fits"), 
