...
def get_fg_catalog2(fg_file):
    """ For the GALEX AIS catalog"""
    data = Table.read(fg_file,format='csv')
    keep,=np.where((data['nuv_artifact'] != 2) & (data['nuv_artifact'] !=64) & (data['fuv']>= 14.5))
    catalog = treecorr.Catalog(ra=data['ra'][keep],dec=data['dec'][keep],ra_units='deg',dec_units='deg')
    #pdb.set_trace()
    return catalog
...
def main(argv):
    ...
    #fg_name = 'galex_AIS/galexAIS_noLgObj_jacquelinemcc.csv'
    ...
    if ortho:
        ...
        dd_outfile = 'dust_correlation_unsubtracted_galexSP.fits'
        dr_outfile = 'dust_correlation_randoms_galexSP.fits'
    ...
    #ax.errorbar(dk['meanR'],dk['kappa'],yerr=dk['sigma'])
    ...
    if ortho:
        ...
        fig.savefig('dust_extinction_galex_superpure.png')
