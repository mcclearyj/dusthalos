...
\begin{enumerate}
	\item Implementation of major modeling updates and code improvements ($\S$\ref{sec:code}), including: 
		...
		\item Implemented a patch-based approach to variance estimation in correlation function calculations, as strongly recommended in TreeCorr documentation. 
		\item Implemented a new dust extinction law taken from Gordon et al. 2023; the one I had been using dated from 2000 and was due for an update. 
		\item Started using the CSFD map recommended in Yi-Kuan Chiang 2023, as CIB contamination in the canonical SFD98 extinction map leads to $10^{-3}-10^{-4}$ mag excesses in E(B-V) (the magnitude of our signal).
		\item Added code to fit power laws to all \av profiles.
		\end{enumerate}
	\item Fit extinction profiles to the updated redMaGiC-background extinction profiles ($\S$\ref{sec:redmagic_results}). The \textit{exponents} are close match to the M\'{e}nard et al.~2010 (M10) result but the \textit{amplitude} is about twice as high. Additionally, we consistently observe a sharp increase in \av at $r < 50$ \hinv kpc that is altogether absent in the  M\'{e}nard result. 
	...
	\item Expanded analysis to galaxies in the Sloan Digital Sky Survey and GALEX-WISE-SDSS Legacy Catalog! The results aren't as amazing as one would hope, but aren't complete messes either. While the WISExSCOS x SDSS results tended to be consistent with the redMaGiC samples at \textit{low} impact parameters, at larger impact parameters ($>$ 10 arcminutes), the extinction flattens out and doesn't decrease. Meanwhile, the GSWLC x SDSS extinction has an interesting dip at intermediate impact parameters but agrees with the redMaGiC curves at high and low impact parameters (Figure \ref{fig:uglycolor}).
	\item Started paper draft! I have a couple of asks of you both, see below. 
\end{enumerate}

I treated this report as an exercise for synthesizing the intense work of the last year as a prelude to writing up the paper, which I believe are now ready to do. Although it seems rather long, half of the page count is appendices with information like full printouts for the power law fits, extra results, and assorted algorithms. You can probably skip it! 
...
\item All: I'd appreciate feedback on how to pitch these results. I am still struggling to think of a decent title!
\end{itemize}
...
also allow us to gain a ``stratospheric perspective'' on intracluster dust, which is known to trace intracluster light and provide a significant source of cooling in the ICM \cite{2022Univ....8..212S}. Inspired by the analysis of the GUViCS study of dust in the Virgo cluster \cite{2020A&A...633L...7L} which used only 12,000 background galaxies, I would like to cross-correlate excess reddening of background galaxies with SuperBIT galaxy cluster members, obtaining results qualitatively similar to Figure \ref{fig:clusterdust}. This would be a lot of fun to try out at some point... I don't know off-hand whether there are any SuperBIT clusters that lie in the DES survey area, but it would be very convenient if they did! 
...
Since update \#1, there have been several useful upgrades and bug fixes to the analysis codebase and improvements to the models used. The major code changes include: mitigation of a bug in random catalog generation; implementing redshift-based matching for assigning photometry to random catalogs; and creating survey masks when not pre-supplied. Model changes were motivated by recent advances in the literature and mainly consist of adopting the CSFD Milky Way extinction correction and an updated dust extinction relation. Code upgrades are summarized in Section \ref{sec:codeupdates}, and upgrades to modeling choices are summarized in Section \ref{sec:modelupdates}.
...
The result is shown in Figure \ref{fig:redshiftmatch}. Note that the photometry catalogs used for this procedure are the color-augmented redMaGiC galaxy catalogs, not the full Y3 GOLD catalog. Using the full Y3 GOLD catalog produces a mismatch in colors, which is attributable to the fact that the redMaGiC galaxies are not a representative sample! It should be noted that this change did not visibly affect extinction profiles. 
...
The TreeCorr package is the backbone of our analysis, so implementing it correctly is crucial if we are to trust our results. Error bars on \av profiles in update \#1 represent the default \texttt{TreeCorr} shot-noise estimate of the covariance, which for \texttt{NKCorrelation} includes Poisson statistics of counting (N) and the ``point variance'' of scalar quantities (K). 
% this is pretty close to TreeCorr copypasta. 
However, this default does not capture \textit{sample} variance, the variation of the signal across the survey area. One should not ignore its contribution, as per the TreeCorr documentation, sample variance generally dominates the total variance at large scales.  Sample variance can be estimated by partitioning the survey area into contiguous patches, performing the measurement separately in each, and then quantifying the variation in the results across the patches.

TreeCorr offers access to this calculation through its \texttt{sample}, \texttt{bootstrap}, and \texttt{marked\_bootstrap} methods for covariance estimation, which are specified in the the \texttt{var\_method} parameter of the \texttt{NKCorrelation} constructor. All of these estimation methods are now accessible in our code via the \texttt{treecorr\_params} block of the \texttt{dust\_calc\_runner.py} configuration file, i.e., \texttt{dust\_calc\_config.yaml}.\footnote{The \texttt{bootstrap} and \texttt{marked\_bootstrap} methods also take a \texttt{num\_bootstrap} parameter that sets the number of bootstrap estimates.} The number of patches into which to divide the survey area is set through \texttt{treecorr\_npatch} in the \texttt{background\_catalog} block of the \texttt{dust\_calc\_runner.py} config, which is passed to the ``signal'' (real background galaxy catalog) \texttt{treecorr.Catalog()} constructor. Since consistency in sample variance estimates across all terms in the reddening estimator requires that the sky patch centers be the same, patch centers for the foreground and random catalogs' \texttt{treecorr.Catalog()} instances are set to the values in the signal.
...
\texttt{src/deredden\_sdss\_magnitudes.py}; the gist of the algorithm is shown in Appendix \ref{appx:deredden}. 

Note that to calculate extinction values correctly, the per-magnitude \av values were multiplied by a factor of 0.78, as described in the Sevilla-Noarbe et al.~2021 \cite{2021ApJS..254...24S}, and the effective wavelengths of each bandpass were rescaled as prescribed in Table 6 of Schlafly \& Finkbeiner 2011 \cite{2011ApJ...737..103S}. As a sanity check, I compared the \texttt{a\_sed\_sfd98\_g} $g$-band extinction value provided in the Y3 GOLD catalog with the value obtained with \texttt{dustmaps.SFD} for one of the redMaGiC galaxy samples. The difference between the two was quite minimal ($-1.86\ee{-9}$), which is reassuring. 
...
To add photometry to the redMaGiC catalogs, I select ``good'' DES Y3 GOLD galaxies with the {Cosmohub}\footnote{\url{https://cosmohub.pic.es/home}} \cite{TALLADA2020100391,2017ehep.confE.488C} query below, then join the two catalogs on the \texttt{COADD\_OBJECT\_ID} parameter. 
...
In addition, CSFD-corrected magnitudes (\texttt{mof\_cm\_mag\_X\_corr\_csfd}) are added to the joined DES Y3 GOLD-redMaGiC background catalogs and replace the \texttt{mof\_cm\_mag\_X} magnitudes used in the first iteration of the analysis.
%
...
After masking to match the WISExSuperCOSMOS catalog and a color cut to simplify the color-redshift space (see $\S$\ref{sec:sdss}), an impressive 16,839,045 galaxies remain. De-reddened magnitudes for our extinction profile analysis are obtained using the CSFD procedure described above. 
...
Random galaxy catalogs are not readily available for either GSWLC or SDSS and so I create them using the updated algorithm described in Section \ref{sec:codeupdates}. The resulting random SDSS catalog has 247,369,950 objects; the random GSWLC catalog contains 167,257,476 objects; and the random WISExSuperCOSMOS catalog for the Northern sky has 5,865,639 objects. 

\subsection{Star catalogs for systematics testing}
Beyond new galaxy catalogs, since update \#1 I have also constructed star catalogs to test for systematic errors in our analysis. The rationale for using star catalogs is thus: unless embedded in a star-forming region, stars do not have extended halos of dust---certainly not at megaparsec scales! Similarly, stars embedded in the Milky Way should not display dust-induced color gradients as a function of projected distance from galaxies outside the Milky Way. So, using star catalogs as both foreground and background samples in the extinction calculation offers a valuable null test of our pipeline. My intuition is that a stellar foreground catalog helps validate the analysis as a whole, while a stellar background catalog with magnitude columns that match the main background sample (i.e., redMaGiC) helps to validate the reddening estimator in particular, or at least the pipeline. 
...
Foreground star catalogs (for cross-correlation with the redMaGiC background catalogs) are selected from the Gaia Data Release 3 database \cite{2023A&A...674A...1G}, accessd via Cosmohub. As elsewhere, overlapping regions are obtained by calculating the intersection of masks for Gaia and DES. As the Gaia collaboration does not make a survey mask readily available, I create one using a simple function (\texttt{radec2healpixels}) described in Section \ref{sec:code:healpix}. The final on-sky overlap between the Gaia and redMaGiC catalogs is shown in Figure \ref{fig:gaiaoverlap}. (Incidentally, the same procedure is carried out to make masks for SDSS and GSWLC catalogs, neither of which have readily available survey masks.) 
...
This finding wasn't guaranteed and is certainly cool to see. However, we also have a couple of new and surprising results: 

\begin{enumerate}
\item For all foreground samples analyzed save the redMaGiC LRG, the \av at $r < 50 h^{-1} kpc$ actually follows a significantly steeper power law than the rest of the profiles. This broken power law result has been consistent since the first preliminary experiments in 2018, so it's no fluke! 
...
I fit power laws to the WISExSuperCOSMOS extinction profiles in the 1-halo regime (50 \hinv kpc $ < r < 1$ \hinv Mpc); the inner limit is set to the point where the redMaGiC LRG extinction and WISExSuperCOSMOS profiles intersect, and the outer limit to the points where the extinction begins to roll off. 
...
The \av profile resulting from cross-correlation of the WISExSuperCOSMOS foreground catalog with an SDSS catalog is shown in Figure \ref{fig:sdss}. I had really wanted this one to work, since this is the same foreground sample we use elsewhere that gave us successful results. And at impact parameters of less than 100 \hinv kpc (2-3 arcminutes), the profile qualitatively matches the redMaGiC-background \av profiles of Figure \ref{fig:redmagicfits}. However, that fat tail past 3 arcminutes is the stickiest result of all of my results. Color cuts on the background galaxies, aggressive masking, etc. do nothing. I had even hoped that CSFD, which is supposedly a superior extinction correction algorithm, would make it go away, but all to no avail. 
...
I find this result to be quite interesting: at low impact parameters (r $<$ 50 \hinv kpc) \textit{and} at high impact parameter/the two-halo regime (r $>$ 1 \hinv Mpc), the \av profile lines up very nicely with the redMaGiC samples, with an amplitude somewhere between redMaGiC LRG and WISExSuperCOSMOS. However, that dip in the middle is somewhat mysterious to me. Its exact location also seems to depend heavily on the NSIDE of the HEALPix mask I create for the catalog. his is a possible indication of my mask being incomplete or wrong; this is why I don't 100\% trust the result. For the sake of completeness, I decided to fit a power law to the outermost six points of this GSWLC \x SDSS extinction profile, with the result shown in Equation \ref{eqn:gswlc}:
...
At this point, I believe that we are ready to publish. One can fiddle with results ad infinitum or try to add new ones, but I am not sure we will gain much.%If you all want to start running systematic analyses, you can, but I am not going to wait around. 
So, I think I'm going to start writing! 

As I work on the draft I would really appreciate suggestions on how to pitch these results. This estimator is novel but the finding is not fundamentally new (unless you count weirdly high \av as new). So, what is the improvement we are making over the state of the art? What have we learned about dust that we didn't know before? Are our findings incompatible with existing dust models? My attempts to link galaxy type and CGM were an attempt to get at some of that deeper knowledge, but those experiments didn't yield satisfactory conclusions and will need to be revisited later. 
