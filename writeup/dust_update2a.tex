...
Given the acknowledged problems of SFD98, the CSFD formulation of XX is the default used here. It may still be interesting to compare the SFD and CSFD outputs. The difference in extinction profiles is shown in Figure \ref{fig:csfdvsnot}. I noticed a couple of interesting trends here. First, there didn't seem to be much of an effect for the high-density sample, but using the SFD extinction correction, the high-lum high-z sample has more extinction at high impact parameters. Put differently, it appears that the SFD prescription added too much extinction at high impact parameters, and with CSFD they are consistent. The interesting/puzzling piece was the redMaGiC LRG 
...
Not CSFD results: had to use a least-squares fit for the LRG, not weighted least squares, since for whatever reason the WLS just didn't want to produce reasonable fits for that sample. 

\begin{verbatim}
Hi-dens WLS fit: 5.86e-03 * (meanr)^-0.62
Hi-z WLS fit: 9.51e-03 * (meanr)^-0.75
Stacked redMaGiC WLS fit: 8.72e-03 * (meanr)^-0.74
